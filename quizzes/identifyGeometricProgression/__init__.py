...
class Question(RandomizedQuestion):
    ...
    def distractors(self,count):
        explanations = []
        for i in range(0,5):
            explanation = 'Most of the terms differ by the common ratio ' + str(self.common_ratio) + ' except that \(a_' + str(i) + r' = ' + str(self.wrong_ones[i][i]) + '\) and \(a_' + str(i+1) + ' = ' + str(self.wrong_ones[i][i+1]) + '\) differ by the ratio ' + str(self.wrong_ones[i][i+1] / self.wrong_ones[i][i]) + ' so this cannot be the beginning of a geometric progression.'
