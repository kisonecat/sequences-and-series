...
\marginnote{With this terminology,
  Theorem~\xrefn{thm:absolute-convergence-implies-convergence} is
  saying that an absolutely convergent series is also a plain old
  convergent series.}

If $\sum_{n=1}^\infty |a_n|$ converges we say that $\sum_{n=1}^\infty a_n$ is
\defnword{absolutely convergent}\index{series!absolute
  convergence}; to say that $\sum_{n=1}^\infty a_n$ converges absolutely is to
...
does not, we say that $\sum_{n=1}^\infty a_n$ converges {\dfont
  conditionally\index{series!conditional convergence}}. For example,
