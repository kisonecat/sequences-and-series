...
Suppose $(a_n)$ is a sequence; then the associated series\sidenote{The
  $\sum$ symbol may look like an E, but it is the Greek letter
  \textit{sigma}, and it makes an S sound---just like the first letter
  of the word ``series.''  If you see ``GR$\Sigma\Sigma$K,'' say ``grsssssk.''}
...
sequence---the \defnword{sequence of partial sums}\index{sequence!of
  partial sums}.  Here's a formal definition.
\begin{definition}
...
\end{align*}
Instead of adding up the infinite sequence $a_n$, we will
look at the sequence of partial sums, and ask whether that sequence of
partial sums converges---and if it converges, then whatever it
converges to---we'll call \textit{that} the value of the series.

\marginnote{This might seem overly complicated, but it solves a serious problem: we no longer are confronted with the \href{http://en.wikipedia.org/wiki/Supertask}{supertask} of adding up infintely many numbers but living to tell the tale.  To take the limit of the sequence of partial sums \textit{is} to add up lots---but not all!---of the terms in the original sequence to see if we're staying close to a particular number, the limit.  That particular number is then, by our definition, declared to be the result of adding up all the terms in the original sequence \textit{in order.}}
...
$\ds\sum_{k=0}^\infty a_0 \, r^k$, we instead\sidenote{Replacing the
  ...
  before in Calculus One.} consider
$$
\lim_{n \to \infty} s_n = \lim_{n \to \infty} \ds\sum_{k=0}^n a_0 \, r^k.
$$
But we can perform some algebraic manipulations on the partial sum.
The trick starts with multiplying $s_n$ by $(1-r)$ to cause some
...
Nevertheless, we can work it out.  The series $\ds\sum_{k=1}^\infty
{1\over 2^k}$ is just like series $\ds\sum_{k=0}^\infty \frac{1}{2^k}$
except the former is missing an initial $k = 0$ term, which is $1/2^0
= 1$.  So each partial sum of the former series is one less than the
corresponding partial sum for the latter series, so the limit is also
one less than the value of the geometric series.  In symbols,
$$
...
\begin{theorem} \relax\label{thm:series-are-linear}x
  Suppose $\sum_{k=0}^\infty a_k$ and $\sum_{k=0}^\infty b_k$ are
...
\begin{definition}
  ...
  \defnword{harmonic series}\index{series!harmonic}.
\end{definition}
...
But in fact the partial sums do go
