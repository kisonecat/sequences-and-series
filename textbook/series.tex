...
\begin{definition}\label{definition:value-of-series}
Consider the series $\ds\sum_{k=1}^\infty a_k$.  This series \defnword{converges}\index{series!convergent}\index{convergent series} 
if the sequence of partial sums $s_n = \sum_{k=1}^n a_k$ converges.  More precisely, if $\ds\lim_{n \to \infty} s_n = L$, we then write
$$
...
is called a \defnword{geometric progression} as we learned back in
...
\marginnote{You might know about a method called \href{http://en.wikipedia.org/wiki/Partial_fraction_decomposition}{\defnword{partial fractions}} to rewrite $\frac{1}{k \cdot (k+1)}$ as a combination of $\frac{1}{k}$ and $\frac{1}{k+1}$.}
...
%BADBAD
%Let's see another example of this trick.

%\begin{example}
%Compute ANOTHER EXAMPLE OF TELESCOPING SERIES
%\end{example}
...
\begin{example}
  \label{example:sum-n-over-n-plus-one-diverges}
...
the harmonic series doesn't diverge for the most obvious reason, but
...
% BADBAD this will be in a video, at least...
