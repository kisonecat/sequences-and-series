...
already seen the sequence $\ds a_i=f(i)=1-1/2^i$, and other examples
are easy to cook up:
\begin{align*}
  a_i &={i\over i+1}, \\
  b_n &={1\over2^n}, \\
...
idea of a real-valued function ``filling in'' the terms of a sequence
when we look at graphs in Section~\xrefn{section:graphs}.

\begin{warning}
  A common misconception is to confuse the sequence with the rule for
  generating the sequence.  The sequence $a_n = (-1)^n$ and the
  sequence $b_n = \cos (\pi \, n)$ are, despite appearances, different
  rules which give rise to the \textit{same} sequence.
\end{warning}
...
In other words, sequences are the same if they output the same list of
real numbers---regardless of whether the given ``rule'' or procedure
for computing those sequences is the same rule.
...
\marginnote{This particular polynomial $n^4 - n + 41$ is rather
...
\begin{solution}
  ...
  like this, this question simply doesn't have an answer.  Or worse,
...
\begin{definition}
  Suppose $a_n$ is a sequence.  We get a
  \defnword{subsequence}\index{sequence!subsequence}\index{subsequence}
  of the sequence $a_n$ if we throw away terms

%  Given a sequence $a_n$, a subsequence  
%	Let be a sequence. When we extract from this sequence only certain elements and drop the remaining ones we obtain a new sequences consisting of an infinite subset of the original sequence. That sequence is called a subsequence and denoted by 
\end{definition}
...
\begin{theorem}
  ...
  If $(b_n)$ is a subsequence of the convergent sequence $(a_n)$, then
  $\ds\lim_{n \to \infty} b_n = \ds\lim_{n \to \infty} a_n$.
\end{theorem}
...
Determine whether the sequence $\ds a_n = \frac{\log n}{n}$ converges or
diverges.  If it converges, compute the limit.
\end{example}
...
Determine whether the sequence $a_n = (-1)^n$ converges or
diverges. If it converges, compute the limit.
\end{example}
...
\marginnote{I imagine that the ``airtight argument'' in the solution
  ...
  now---we'll have more opportunities for doing this sort of
...
We will not prove this theorem in the textbook\sidenote{Proving this
  ...
  glance'' at sequences---so much be left unsaid.} Nevertheless, it is
not hard to believe: suppose that a sequence is increasing and
...
``eventually'' monotonic\sidenote{After all, the limit of a sequence
  is the same as that of a subsequence, and the ``tail'' of a sequence
  is an example of a subsequence.} that is, that at some point it
becomes increasing or decreasing.

%BADBAD
For example, the sequence $10$, $9$, $8$,
...
Show that the sequence $a_n = n^{1/n}$ converges.
\end{example}
...
\marginnote{As it happens, you could compute the limit in
  Example~\xrefn{example:nth-root-of-n}, but the solution shows that
